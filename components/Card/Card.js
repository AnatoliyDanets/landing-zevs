...
export default function Card({ card, onClick, disabled, text, handleChange, currentSize, sizes }) {
    return (
        <>
            <article className={s.card__article} >
                <h3 style={{ color: "#d2ba40", textAlign: "center", fontSize: "24px", marginBottom: "20px" }}> {card.model}</h3>
                                ...
                                <FormControl sx={{ minWidth: 120 }}>
                                    ...
                                    >
                                        {sizes.map(({ id, width, size }) => (
                                            <MenuItem key={id} value={+width}>{size}</MenuItem>
                                        ))}
